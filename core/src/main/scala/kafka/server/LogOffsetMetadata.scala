 ...
 *  2. the base message offset of the located segment 保存该位移值所在日志段的起始位移,判断两条消息是否处于同一个日志段的
 *  3. the physical position on the located segment 保存该位移值所在日志段的物理磁盘位置,计算两个位移值之间的物理磁盘位置差值
 */
case class LogOffsetMetadata(messageOffset: Long,
  ...
  // check if this offset is on the same segment with the given offset  判断给定的两个 LogOffsetMetadata 对象是否处于同一个日志段的
