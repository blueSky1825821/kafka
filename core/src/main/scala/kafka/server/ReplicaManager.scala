...
class ReplicaManager(val config: KafkaConfig,
  ...
  def appendRecords(timeout: Long,// 请求处理超时时间
                    ...
                    internalTopicsAllowed: Boolean,// 是否允许写入内部主题，普通生产者不可写，Coordinator可写
            ...
            partitionOpt.foreach { partition =>
              ...
              if (!hasConsistentTopicId(requestTopicId, logTopicId)) {
                ...
                if (partitionState.replicas.contains(localBrokerId)) {
                  ...
                  } else {// 确定Broker上副本是哪些分区的Follower副本
  ...
  private def makeFollowers(controllerId: Int,
    ...
    try {
      ...
      if (isShuttingDown.get()) {
        ...
        // 为需要将当前Broker设置为Follower副本的分区 // 确定Leader Broker和起始读取位移值fetchOffset
