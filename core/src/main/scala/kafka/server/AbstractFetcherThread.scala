...
//类似于一个快捷方式：以后凡是源码中需要用到 FetchResponse.PartitionData[Records]的地方，都可以简单地使用 FetchData 替换掉
  ...
  protected def buildFetch(partitionMap: Map[TopicPartition, PartitionFetchState]  // 一组要读取的分区列表  // 分区是否可读取取决于PartitionFetchState中的状态
                          ): ResultWithPartitions[Option[ReplicaFetch]]// 封装FetchRequest.Builder对象
...
case class PartitionFetchState(topicId: Option[Uuid],
  ...
  // 分区被推迟获取数据的条件：存在未过期的延迟任务，是用于判断是否需要推迟获取对应分区的消息
