...
class SocketServer(val config: KafkaConfig,
  ...
  private def startDataPlaneProcessorsAndAcceptors(authorizerFutures: Map[Endpoint, CompletableFuture[Void]]): Unit = {
    val interBrokerListener = dataPlaneAcceptors.asScala.keySet// 获取Broker间通讯所用的监听器，默认是PLAINTEXT
  ...
  private def createControlPlaneAcceptorAndProcessor(endpointOpt: Option[EndPoint]): Unit = {
    endpointOpt.foreach { endpoint =>// 遍历监听器集合
      connectionQuotas.addListener(config, endpoint.listenerName)// 将监听器纳入到连接配额管理之下
      val controlPlaneAcceptor = createControlPlaneAcceptor(endpoint, controlPlaneRequestChannelOpt.get)// 为监听器创建对应的Acceptor线程
...
private[kafka] abstract class Acceptor(val socketServer: SocketServer,
                                       val endPoint: EndPoint,//Kafka Broker 连接信息
  ...
  private val sendBufferSize = config.socketSendBufferBytes//SocketOptions 的 SO_SNDBUF，即用于设置出站（Outbound）网络 I/O 的底层缓冲区大小。该值默认是 Broker 端参数 socket.send.buffer.bytes 的值，即 100KB。
  private val recvBufferSize = config.socketReceiveBufferBytes//SocketOptions 的 SO_RCVBUF，即用于设置入站（Inbound）网络 I/O 的底层缓冲区大小。该值默认是 Broker 端参数 socket.receive.buffer.bytes 的值，即 100KB。
  private val listenBacklogSize = config.socketListenBacklogSize

  private val nioSelector = NSelector.open()//Selector对象负责执行底层实际I/O操作，如监听连接创建请求、读写请求等
  private[network] val serverChannel = openServerSocket(endPoint.host, endPoint.port, listenBacklogSize)//roker端创建对应的ServerSocketChannel实例,后续把该Channel向上一步的Selector对象注册
  private[network] val processors = new ArrayBuffer[Processor]()//Processor线程数组
  ...
  private def startProcessors(processors: Seq[Processor]): Unit = synchronized {
    processors.foreach { processor =>
      ...
      // 假设为序号为0的Broker设置PLAINTEXT:
      // localhost:9092作为连接信息，那么3个Processor线程名称分别为：
  ...
  private def acceptNewConnections(): Unit = {
    val ready = nioSelector.select(500)// 每500毫秒获取一次就绪I/O事件
    if (ready > 0) {// 如果有I/O事件准备就绪
      ...
      while (iter.hasNext && isRunning) {
        try {
          ...
          if (key.isAcceptable) {
            accept(key).foreach { socketChannel =>// 调用accept方法创建Socket连接
              ...
              do {
                ...
                processor = synchronized {
                  ...
                  currentProcessorIndex = currentProcessorIndex % processors.length// 指定由哪个Processor线程进行处理
                  processors(currentProcessorIndex)
                }
                currentProcessorIndex += 1// 更新Processor线程序号
              } while (!assignNewConnection(socketChannel, processor, retriesLeft == 0))//Processor 线程创建与发送方的连接
            }
          } else
...
private[kafka] class Processor(val id: Int,
  ...
  private val newConnections = new ArrayBlockingQueue[SocketChannel](connectionQueueSize)//创建的新连接信息
  private val inflightResponses = mutable.Map[String, RequestChannel.Response]()//临时 Response 队列。当 Processor 线程将 Response 返还给 Request 发送方之后，还要将 Response 放入这个临时队列，回调逻辑处理
  private val responseQueue = new LinkedBlockingDeque[RequestChannel.Response]()//每个 Processor 线程都会维护自己的 Response 队列
  ...
  private def poll(): Unit = {
    ...
    try selector.poll(pollTimeout)//执行那些准备就绪的 I/O 操作，不管是接收 Request，还是发送 Response
    catch {
  ...
  private def processCompletedReceives(): Unit = {
    selector.completedReceives.forEach { receive =>// 遍历所有已接收的Request
      try {
        openOrClosingChannel(receive.source) match {// 保证对应连接通道已经建立
          case Some(channel) =>
            ...
            if (header.apiKey == ApiKeys.SASL_HANDSHAKE && channel.maybeBeginServerReauthentication(receive,
              ...
              if (channel.serverAuthenticationSessionExpired(nowNanos)) {// 如果认证会话已过期，则关闭连接
                ...
                requestChannel.sendRequest(req)// 核心代码：将Request添加到Request队列
  ...
  private def processCompletedSends(): Unit = {
    selector.completedSends.forEach { send =>// 遍历底层SocketChannel已发送的Response
      try {
        val response = inflightResponses.remove(send.destinationId).getOrElse {// 取出对应inflightResponses中的Response
  ...
  private def processDisconnected(): Unit = {
    selector.disconnected.keySet.forEach { connectionId =>// 遍历底层SocketChannel的那些已经断开的连接
      try {
  ...
  private def configureNewConnections(): Unit = {
    ...
    while (connectionsProcessed < connectionQueueSize && !newConnections.isEmpty) {// 如果没超配额并且有待处理新连接
      ...
      try {
        ...
        selector.register(connectionId(channel.socket), channel)//用给定Selector注册该Channel
