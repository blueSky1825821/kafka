...
object RequestChannel extends Logging {
  ...
  class Request(val processor: Int,//Processor 线程的序号
                val context: RequestContext,//标识请求上下文信息的
                val startTimeNanos: Long,//Request 对象被创建的时间，主要用于各种时间统计指标的计算。
                val memoryPool: MemoryPool,//定义的一个非阻塞式的内存缓冲区，主要作用是避免 Request 对象无限使用内存。
                @volatile var buffer: ByteBuffer,//真正保存 Request 对象内容的字节缓冲区,org.apache.kafka.common.requests.AbstractRequest.doParseRequest
...
//用于通知 Broker 的 Socket Server 组件,某个 TCP 连接通信通道开始被限流（throttling）
...
class RequestChannel(val queueSize: Int,//队列最大长度
  ...
  private val processors = new ConcurrentHashMap[Int, Processor]()//Key 就是前面我们说的 processor 序号，而 Value 则对应具体的 Processor 线程对象。
...
object RequestMetrics {
  ...
  val RequestQueueTimeMs = "RequestQueueTimeMs"//计算 Request 在 Request 队列中的平均等候时间
  val LocalTimeMs = "LocalTimeMs"//计算 Request 实际被处理的时间
  val RemoteTimeMs = "RemoteTimeMs"//读写请求（PRODUCE 请求和 FETCH 请求）逻辑涉及等待其他 Broker 操作的步骤
