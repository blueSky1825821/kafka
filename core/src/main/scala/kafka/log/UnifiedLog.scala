        ...
        maybeHandleIOException(s"Error while appending records to $topicPartition in dir ${dir.getParent}") {
          ...
          maybeDuplicate match {
            ...
            case None =>
              ...
              // update the producer state 未保存的事务幂等记录最多5条
              ...
              // 是否需要手动落盘。一般情况下我们不需要设置Broker端参数log.flush.interval.messages // 落盘操作交由操作系统来完成。但某些情况下，可以设置该参数来确保高可靠性
...
object UnifiedLog extends Logging {//日志文件
  ...
  val ProducerSnapshotFileSuffix = ".snapshot"//Kafka 为幂等型或事务型 Producer 所做的快照文件

  val TxnIndexFileSuffix = LocalLog.TxnIndexFileSuffix

  val DeletedFileSuffix = LocalLog.DeletedFileSuffix//删除日志段操作创建的文件

  val CleanedFileSuffix = LocalLog.CleanedFileSuffix

  val SwapFileSuffix = LocalLog.SwapFileSuffix

  val DeleteDirSuffix = LocalLog.DeleteDirSuffix//当你删除一个主题的时候，主题的分区文件夹会被加上这个后缀

  val FutureDirSuffix = LocalLog.FutureDirSuffix//用于变更主题分区文件夹地址的
